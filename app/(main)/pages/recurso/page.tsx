...
const Recurso= () => {
    ...
    const [recursos, setRecursos] = useState<Agenda.Recurso[] | null>(null);
    ...
    const [recurso, setRecurso] = useState<Agenda.Recurso>(recursoVazio);
    const [selectedRecursos, setSelectedRecursos] = useState<Agenda.Recurso[]>([]);
    ...
    const editRecurso = (recurso: Agenda.Recurso) => {
        setRecurso({ ...recurso });
        setRecursoDialog(true);
    };

    const confirmDeleteRecurso = (recurso: Agenda.Recurso) => {
        setRecurso(recurso);
            ...
            recursoService.excluir(recurso.id)
                ...
                .catch((error)=>{
                    toast.current?.show({
                        severity: 'error',
                        summary: 'Error!',
                        detail: 'Error ao deletar o recurso!',
                        life: 3000
                    });
    ...
    const deleteSelectedRecursos = () => {
        ...
        Promise.all(selectedRecursos.map(async(_recurso) => {
            ...
            toast.current?.show({
                    severity: 'error',
                    summary: 'Error!',
                    detail: 'Error ao deletar o recursos!',
                    life: 3000
                });
        })
    };
    ...
    const idBodyTemplate = (rowData: Agenda.Recurso) => {
        return (
            <>
    ...
    const nomeBodyTemplate = (rowData: Agenda.Recurso) => {
        return (
            <>
    ...
    const chaveBodyTemplate = (rowData: Agenda.Recurso) => {
        return (
            <>
    ...
    const recursoDialogFooter = (
        <>
            ...
            <Button label="Savar" icon="pi pi-check" text onClick={saveRecurso} />
        </>
    );
                ...
                <div className="card">
                    ...
                    <Dialog visible={deleteRecursoDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteRecursoDialogFooter} onHide={hideDeleteRecursoDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {recurso && (
                                <span>
                                    Você realmente deseja excluir recurso. <b>{recurso.nome}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
